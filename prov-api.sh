...
operation_parameters_minimum_occurrences["exploreDownstream:::starting_id"]=1
operation_parameters_minimum_occurrences["exploreDownstream:::depth"]=0
operation_parameters_minimum_occurrences["exploreUpstream:::starting_id"]=1
operation_parameters_minimum_occurrences["exploreUpstream:::depth"]=0
operation_parameters_minimum_occurrences["registerCompleteModelRun:::ModelRunRecord"]=1
...
operation_parameters_maximum_occurrences["exploreDownstream:::starting_id"]=0
operation_parameters_maximum_occurrences["exploreDownstream:::depth"]=0
operation_parameters_maximum_occurrences["exploreUpstream:::starting_id"]=0
operation_parameters_maximum_occurrences["exploreUpstream:::depth"]=0
operation_parameters_maximum_occurrences["registerCompleteModelRun:::ModelRunRecord"]=0

##
# The type of collection for specifying multiple values for parameter:
# - multi, csv, ssv, tsv
declare -A operation_parameters_collection_type
operation_parameters_collection_type["exploreDownstream:::starting_id"]=""
operation_parameters_collection_type["exploreDownstream:::depth"]=""
operation_parameters_collection_type["exploreUpstream:::starting_id"]=""
operation_parameters_collection_type["exploreUpstream:::depth"]=""
operation_parameters_collection_type["registerCompleteModelRun:::ModelRunRecord"]=""
...
${BOLD}${WHITE}Usage${OFF}
  ...
  - ${CYAN}<url>${OFF} - endpoint of the REST service without basepath
           Can also be specified in PROV_API_HOST environment variable.
...
echo "  $ops" | column -t -s ';'
    echo ""
    echo -e "${BOLD}${WHITE}[exploreLineage]${OFF}"
read -r -d '' ops <<EOF
  ${CYAN}exploreDownstream${OFF};Explore Downstream (AUTH)
  ${CYAN}exploreUpstream${OFF};Explore Upstream (AUTH)
EOF
echo "  $ops" | column -t -s ';'
    echo ""
    echo -e "${BOLD}${WHITE}[modelRuns]${OFF}"
read -r -d '' ops <<EOF
  ${CYAN}registerCompleteModelRun${OFF};Register Model Run Complete (AUTH)
EOF
echo "  $ops" | column -t -s ';'
    echo ""
...
##############################################################################
#
# Print help for exploreDownstream operation
#
##############################################################################
print_exploreDownstream_help() {
    echo ""
    echo -e "${BOLD}${WHITE}exploreDownstream - Explore Downstream${OFF}${BLUE}(AUTH - OAuth2)${OFF}" | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e ""
    echo -e "explore_downstream
...
--------" | paste -sd' ' | fold -sw 80
    ...
    echo -e "  * ${GREEN}starting_id${OFF} ${BLUE}[string]${OFF} ${RED}(required)${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: starting_id=value${OFF}" \
        | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e "  * ${GREEN}depth${OFF} ${BLUE}[integer]${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: depth=value${OFF}" \
        | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo ""
    echo -e "${BOLD}${WHITE}Responses${OFF}"
    code=200
...
##############################################################################
#
# Print help for exploreUpstream operation
#
##############################################################################
print_exploreUpstream_help() {
    echo ""
    echo -e "${BOLD}${WHITE}exploreUpstream - Explore Upstream${OFF}${BLUE}(AUTH - OAuth2)${OFF}" | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e ""
    echo -e "explore_upstream
...
##############################################################################
#
# Print help for registerCompleteModelRun operation
#
##############################################################################
print_registerCompleteModelRun_help() {
    echo ""
    echo -e "${BOLD}${WHITE}registerCompleteModelRun - Register Model Run Complete${OFF}${BLUE}(AUTH - OAuth2)${OFF}" | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e ""
    echo -e "register_model_run_complete
...
##############################################################################
#
# Call exploreDownstream operation
#
##############################################################################
call_exploreDownstream() {
    # ignore error about 'path_parameter_names' being unused; passed by reference
    # shellcheck disable=SC2034
    local path_parameter_names=()
    # ignore error about 'query_parameter_names' being unused; passed by reference
    # shellcheck disable=SC2034
    local query_parameter_names=(starting_id depth  )
    local path

    if ! path=$(build_request_path "/explore/downstream" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
##############################################################################
#
# Call exploreUpstream operation
#
##############################################################################
call_exploreUpstream() {
    # ignore error about 'path_parameter_names' being unused; passed by reference
    # shellcheck disable=SC2034
    local path_parameter_names=()
    # ignore error about 'query_parameter_names' being unused; passed by reference
    ...
    if ! path=$(build_request_path "/explore/upstream" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
##############################################################################
#
# Call registerCompleteModelRun operation
#
##############################################################################
call_registerCompleteModelRun() {
    # ignore error about 'path_parameter_names' being unused; passed by reference
    # shellcheck disable=SC2034
    local path_parameter_names=()
    # ignore error about 'query_parameter_names' being unused; passed by reference
    ...
    if ! path=$(build_request_path "/model_run/register_complete" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="POST"
...
case $key in
    ...
    ;;
    exploreDownstream)
    operation="exploreDownstream"
    ;;
    exploreUpstream)
    operation="exploreUpstream"
    ;;
    registerCompleteModelRun)
    operation="registerCompleteModelRun"
    ;;
    *==*)
...
if [[ -z "$host" ]]; then
    ERROR_MSG="ERROR: No hostname provided!!! Define env variable PROV_API_HOST like 'export PROV_API_HOST=...' or provide on command line option '--host ...'"
    exit 1
fi
